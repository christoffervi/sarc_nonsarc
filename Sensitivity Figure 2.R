...
spl <- as_tibble(survSplit(Surv(first_encounter_age, t2_af,
                                event = event_af)~sarc_status, data = dfposnegproband %>% filter(t2_af>first_encounter_age),
                           cut = c(30,45,55,65), 
                           episode = "agegroup")) %>%
...
spl <- as_tibble(survSplit(Surv(first_encounter_age, t2_vt,
                                event = event_vt)~sarc_status, data = dfposnegproband %>% filter(t2_vt>first_encounter_age),
                           cut = c(30,45,55,65), 
                           episode = "agegroup")) %>%
  #Calculate time for each age group and event type

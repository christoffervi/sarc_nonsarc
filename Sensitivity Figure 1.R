...
dfposnegproband <- 
  dfposneg %>% filter(!presentation_reason %in% c('Family Screening'))

 # dfposnegproband <- 
 #   dfposneg %>% filter(!str_detect(presentation_reason, 'Family Screening')| is.na(presentation_reason))

 # dfposnegproband <- 
 #   dfposneg %>% filter(!str_detect(presentation_reason, 'Family Screening')| if_else(is.na(presentation_reason) & is_proband==1,1,0)==1)
 

dfnegproband <- 
  dfposnegproband %>% filter(sarc_status=="SARC(-)")
dfposproband <- 
  dfposnegproband %>% filter(sarc_status=="SARC(+)")
...
rr_df<-
  rbind(rr_func(dfposnegproband, sarc_status, event_death, "All-cause mortality"),
        rr_func(dfposnegproband, sarc_status, event_hcm_death, "HCM-related mortality"),
        rr_func(dfposnegproband, sarc_status, obese, "Obesity"),
        # rr_func(dfposnegproband, sarc_status, event_ablation, "Ablation"),
        rr_func(dfposnegproband, sarc_status, event_arrhythmia_a_fib, "Atrial Fibrillation"),
        rr_func(dfposnegproband, sarc_status, event_arrhythmia_nsvt, "NSVT"),
        rr_func(dfposnegproband, sarc_status, event_cardiac_arrest, "Cardiac Arrest"),
        rr_func(dfposnegproband, sarc_status, event_htn, "Hypertension"),
        rr_func(dfposnegproband, sarc_status, event_icd, "ICD implantation"),
        rr_func(dfposnegproband, sarc_status, event_lvsd, "LVSD"),
        rr_func(dfposnegproband, sarc_status, event_nyha_hf, "NYHA III-IV"),
        rr_func(dfposnegproband, sarc_status, event_obstruction, "Obstruction"),
        rr_func(dfposnegproband, sarc_status, event_srt, "SRT"),
        rr_func(dfposnegproband, sarc_status, event_composite_v_arrhythmia, "Composite VA"),
        rr_func(dfposnegproband, sarc_status, event_stroke, "Stroke"),
        rr_func(dfposnegproband, sarc_status, event_htxvad, "Cardiac transplantation")  
  ) %>%
  left_join( 
    bind_rows(rr_p(dfposnegproband, sarc_status, event_death, "All-cause mortality"),
              rr_p(dfposnegproband, sarc_status, event_hcm_death, "HCM-related mortality"),
              rr_p(dfposnegproband, sarc_status, obese, "Obesity"),
              # rr_p(dfposnegproband, sarc_status, event_ablation, "Ablation"),
              rr_p(dfposnegproband, sarc_status, event_arrhythmia_a_fib, "Atrial Fibrillation"),
              rr_p(dfposnegproband, sarc_status, event_arrhythmia_nsvt, "NSVT"),
              rr_p(dfposnegproband, sarc_status, event_cardiac_arrest, "Cardiac Arrest"),
              rr_p(dfposnegproband, sarc_status, event_htn, "Hypertension"),
              rr_p(dfposnegproband, sarc_status, event_icd, "ICD implantation"),
              rr_p(dfposnegproband, sarc_status, event_lvsd, "LVSD"),
              rr_p(dfposnegproband, sarc_status, event_nyha_hf, "NYHA III-IV"),
              rr_p(dfposnegproband, sarc_status, event_obstruction, "Obstruction"),
              rr_p(dfposnegproband, sarc_status, event_srt, "SRT"),
              rr_p(dfposnegproband, sarc_status, event_composite_v_arrhythmia, "Composite VA"),
              rr_p(dfposnegproband, sarc_status, event_stroke, "Stroke"),
              rr_p(dfposnegproband, sarc_status, event_htxvad, "Cardiac transplantation")  
...
rr_df %>% 
  ...
  geom_segment(aes(x=.47, xend=4.8, y=term, yend= term), color = "gray", linetype = 3)+
  ...
  geom_text(aes(x= 18, y = term, label = round(.est, 2)), size = 3.5, vjust = 0.3)+
  ...
  coord_cartesian(xlim = c(.47,24), ylim=c(.6,17), expand = F, clip = "off")

p2
ggsave(filename = "Circ - Figure 1 - sensitivity.tiff", compression = "lzw", height = 16, width = 20, units = "cm", dpi =2200)    
...
ggsave(filename = "Circ - Figure S1.tiff", compression = "lzw", height = 32, width = 20, units = "cm", dpi =2200)    

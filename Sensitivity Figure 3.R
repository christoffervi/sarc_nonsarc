hf<- dfposnegproband %>% mutate(event_hf_death = if_else(death_cause=="Heart Failure",1,0),
...
dfposnegproband<- dfposnegproband %>% mutate(event_hf_death = if_else(death_cause=="Heart Failure",1,0),
                               death_causes = case_when(str_detect(death_cause, "Non-Card|Infect|Unkn|Proce|Acci|GI")~"Non-cardiovascular death",
                                                        str_detect(death_cause, "SCD")~"Sudden cardiac death",
                                                        str_detect(death_cause, "Myoca|Cardio|CAD")~"Other cardiovascular death",
                                                        str_detect(death_cause, "Fail|HCM")~"Heart failure",
...
spl <- as_tibble(survSplit(Surv(first_encounter_age, t2_death,
                                event = event_hcm_death)~sarc_status, data = dfposnegproband %>% filter(t2_death>first_encounter_age),
                           cut = c(30,45,55,65), 
                           episode = "agegroup")) %>%
...
ggsave(filename = "sensitivty Figure 3 - 2025-03-06.tiff", compression = 'lzw', height = 22, width = 16, units = "cm", dpi =2000)
...
glm(event_hcm_death~sarc_status, data = dfposnegproband, family = 'binomial') %>% tidy(exponentiate =T, conf.int=T)

...
spl <- as_tibble(survSplit(Surv(first_encounter_age, t2_lvsd,
                                event = event_lvsd)~sarc_status, data = dfposneg %>% filter(t2_lvsd>first_encounter_age),
                           cut = c(30,45,55,65), 
                           episode = "agegroup")) %>%
  #Calculate time for each age group and event type
  mutate(time = t2_lvsd-first_encounter_age) %>% 
...
ggsave(filename = "lvsd_age.pdf", device = cairo_pdf, height = 26, width = 20, units = "cm", dpi =2900)

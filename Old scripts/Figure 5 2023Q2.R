hf<- dfposneg %>% mutate(event_hf_death = if_else(death_cause=="Heart Failure",1,0),
...
q1<-
  ...
  geom_point(position = position_dodge(width = .3), shape = c(21,21,21,21,21,21,21,21,21,21,22,22), size = 4,
             show.legend = F)+
  scale_fill_manual(values = chris_colors_3[2:3])+ #scale_fill_scico_d(palette = "batlow")+
  scale_y_continuous(n.breaks = 6)+
  #scale_y_log10()+
  coord_cartesian(xlim = c(.6,6.4), ylim = c(-0,
     #                                        min(dat.df$lower)*1000,
                                             trunc(max(dat.df$upper)*1000)+.1), expand = F, clip = "off")+
...
q2<-
  ...
  scale_color_manual(values = chris_colors_3[2:3])+ #scale_fill_scico_d(palette = "batlow")+
...
q4<-
  ...
  # geom_ribbon(aes(fill = strata),alpha = .25, show.legend=F, size =0)+
  #  annotate("text", x= 7.8, y = c(0.042, 0.01), label = c("Sarc+", "Sarc-"),
  #           color = chris_colors_3[2:3], #scico(2, palette = "batlow", direction = -1), angle= 20,
  #           family = "Roboto")+
  annotate("text", x= 7.8, y = c( 0.015,0.03), label = c("Sarc-", "Sarc+"),
           color = c(chris_colors_3[2:3]), #scico(2, palette = "batlow", direction = -1),
           angle= 20,
  ...
  scale_fill_manual(values = chris_colors_3[2:3])+ #scale_fill_scico_d()+
  scale_color_manual(values = chris_colors_3[2:3])+ #scale_fill_scico_d(palette = "batlow")+
...
q5<-
  ...
  theme_void()+
  scale_color_manual(values = chris_colors_3[2:3])+ #scale_color_scico_d()+
...
ggsave(filename = "Figure 5 - 2023Q2.tiff", compression = 'lzw', height = 22, width = 16, units = "cm", dpi =2000)

...
rr_df<-
  rbind(rr_func(dfposneg, sarc_status, event_death, "All-cause mortality"),
      ...
      #  rr_func(dfposneg, sarc_status, event_ablation, "Ablation"),
        ...
        rr_func(dfposneg, sarc_status, event_composite_v_arrhythmia, "Composite VA"),
...
rr_df %>% 
  mutate(term_cat = case_when( term %in% c("All-cause mortality","HCM-related mortality")~'Mortality',
                               ...
                               term %in% c("Obesity","Hypertension")~'Cardiovascular comorbidities',
                               term %in% c("LVSD","Cardiac transplantation", 'NYHA III-IV')~'Heart Failure',
                               term %in% c("Stroke")~'Stroke',
                               term %in% c("Obstruction", 'SRT')~'Cardiac remodeling',
                               T~'VUF'
  ),
  term_cat = factor(term_cat, levels = c('Cardiovascular comorbidities','Cardiac remodeling','Heart Failure','Arrhythmias','Stroke','Mortality')))
rr_df %>% mutate(.est = if_else(rowname=="sarc(-)",NA,.est),
  ...
  geom_point(size = 4, shape= 21, show.legend = T)+
  labs(x = "Relative Risk Ratio", y="")+
#  scale_fill_scico(palette = "berlin")+
  scale_fill_manual(values = chris_colors_3)+
  scale_y_discrete()+
  ...
  coord_cartesian(xlim = c(.47,12), ylim=c(.6,17), expand = F, clip = "off")
ggsave(filename = "Figure 1 2023Q2.pdf", device = cairo_pdf, height = 16, width = 20, units = "cm", dpi =2900)    

...
spl <- as_tibble(survSplit(Surv(first_encounter_age, t2_stroke,
                                event = event_stroke)~sarc_status, data = dfposneg %>% filter(t2_stroke>first_encounter_age),
                           cut = c(30,45,55,65), 
                           episode = "agegroup")) %>%
  #Calculate time for each age group and event type
  mutate(time = t2_stroke-first_encounter_age) %>% 
  group_by(agegroup, sarc_status) %>% 
  summarise(nyear = sum(time), ncas = sum(event_stroke)) %>% 
  ungroup() 
...
p4<-
  hf %>% 
  mutate(time1 = t2_stroke-first_encounter_age ) %>% 
  filter(time1>0) %>% 
  surv_fit(Surv(time1, event_stroke)~sarc_status, data = .) %>% broom::tidy() %>% 
  ...
  coord_cartesian(xlim = c(0,10), 
                  ylim= c(0,hf %>% 
                            mutate(time1 = t2_stroke-first_encounter_age ) %>% 
                            filter(time1>0) %>% 
                            surv_fit(Surv(time1, event_stroke)~sarc_status, data = .) %>% 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.text.x = element_text(family = "Helvetica", size = 10),
        axis.text.y = element_text(family = "Helvetica", size = 10))+
  annotate("text", x= .5,y=.045, hjust =0, family = "Helvetica",
           label = glue::glue("log rank {surv_pvalue(hf %>% 
                mutate(time1 = t2_stroke-first_encounter_age ) %>% 
                filter(time1>0) %>% 
                surv_fit(Surv(time1, event_stroke)~sarc_status, data = .))[,4]}"))
p5<-
  hf %>% 
  mutate(time1 = t2_stroke-first_encounter_age ) %>% 
  filter(time1>0) %>% 
...
ggsave(filename = "stroke_age.tiff", compression = "lzw", height = 26, width = 20, units = "cm", dpi =900)

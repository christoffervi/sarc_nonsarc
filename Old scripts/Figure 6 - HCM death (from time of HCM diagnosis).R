...
spl <- as_tibble(survSplit(Surv(primary_diagnosis_age, t2_death,
                                event = event_hcm_death)~sarc_status, data = dfposneg %>% filter(t2_death>primary_diagnosis_age),
                           cut = c(30,45,55,65), 
                           episode = "agegroup")) %>%
  #Calculate time for each age group and event type
  mutate(time = t2_death-primary_diagnosis_age) %>% 
...
p1<-
  ...
  coord_cartesian(xlim = c(.6,6.4), ylim = c(0,20), expand = F, clip = "off")+
...
p4<-
  hf %>% 
  mutate(time1 = t2_death-primary_diagnosis_age ) %>% 
  filter(time1>0) %>% 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.text.x = element_text(family = "Helvetica", size = 10),
        axis.text.y = element_text(family = "Helvetica", size = 10))+
  annotate("text", x= .5,y=.0125, hjust =0, family = "Helvetica",
...
ggsave(filename = "hcm_death_age_from_diagnosis.tiff", compression = "lzw", height = 26, width = 20, units = "cm", dpi =900)

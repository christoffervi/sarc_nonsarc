...
spl <- as_tibble(survSplit(Surv(first_encounter_age, t2_htxvad,
                                event = event_htxvad)~sarc_status, data = dfposneg %>% filter(t2_htxvad>first_encounter_age),
                           cut = c(30,45,55,65), 
                           episode = "agegroup")) %>%
  #Calculate time for each age group and event type
  mutate(time = t2_htxvad-first_encounter_age) %>% 
  group_by(agegroup, sarc_status) %>% 
  summarise(nyear = sum(time), ncas = sum(event_htxvad)) %>% 
  ungroup() 
...
r4<-
  hf %>% 
  mutate(time1 = t2_htxvad-first_encounter_age ) %>% 
  filter(time1>0 & time1<10.2) %>% 
  surv_fit(Surv(time1, event_htxvad)~sarc_status, data = .) %>% broom::tidy() %>% 
  ...
  theme(panel.background = element_rect(fill = "white"),
        ...
        axis.text.x = element_text(family = "Helvetica", size = 10),
        axis.text.y = element_text(family = "Helvetica", size = 10))+
  annotate("text", x= .5,y=.015, hjust =0, family = "Helvetica",
           label = glue::glue("log rank {surv_pvalue(hf %>% 
                mutate(time1 = t2_htxvad-first_encounter_age ) %>% 
                filter(time1>0) %>% 
                surv_fit(Surv(time1, event_htxvad)~sarc_status, data = .))[,4]}"))
r5<-
  hf %>% 
  mutate(time1 = t2_htxvad-first_encounter_age ) %>% 
  filter(time1>0) %>% 
...
r4/r5/r1/r2/r3+plot_layout(heights = c(3,1,3,.75,.75))+plot_annotation(tag_levels = list(c("A","","B","","")))
ggsave(filename = "cardiac tx_age.tiff", compression = "lzw", height = 26, width = 20, units = "cm", dpi =900)
